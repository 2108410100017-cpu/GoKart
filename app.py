...
app = FastAPI(
    ...
    description="A minimal end-to-end ML feedback loop",
    version="1.0"
)
...
# -------------------------------
# Helper: Load latest model
# -------------------------------
def load_latest_model():
    model_files = glob.glob("model_v*.pkl")
    if not model_files:
        raise HTTPException(status_code=500, detail="No trained model found. Run initial training first.")
...
def predict(data: PredictionInput):
    ...
    return {"prediction": prediction, "model_version": f"v{version}"}
...
def simulate_ground_truth():
    ...
    for entry in lines:
        ...
        # Same probabilistic logic as initial data generation
    ...
    # Append to cumulative processed data
    with open("processed_data.jsonl", "a") as f:
        for entry in processed:
            f.write(json.dumps(entry) + "\n")
    
    # Clear live data for next cycle
    open("live_data.jsonl", "w").close()
    
    return {"status": "success", "processed_entries": len(processed)}
...
def retrain():
    ...
    if candidate_acc > current_acc:
        ...
        joblib.dump(candidate, f"model_v{new_version}.pkl")
        result["new_model_saved"] = f"model_v{new_version}.pkl"
    else:
        result["new_model_saved"] = None
        result["message"] = "No improvement - keeping current model"
...
def run_full_cycle():
    ...
    ds = create_dataset()
    rt = retrain()
    return {"ground_truth": gt, "dataset": ds, "retrain": rt}
...
def root():
    ...
    return {
        ...
        "endpoints": {
            "POST /predict": "Get prediction (send temperature & time_of_day)",
            "POST /cycle/run-full": "Run the complete daily feedback loop",
